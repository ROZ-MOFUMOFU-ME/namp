...
const JobManager = function JobManager(this: any, options: any) {
    ...
    // VIPSTARCOIN / HTMLcoin (qtum-style) block templates MUST carry
    // hashstateroot + hashutxoroot: they are part of the 181-byte header the
    // canonical miner (ccminer -a vipstar / -a html) hashes. If a
    // getblocktemplate response omits them (a daemon hiccup or mid-reorg
    // partial template), blockTemplate.getJobParams() silently falls back to a
    // plain 80-byte job — which the miner cannot reconstruct, so every share it
    // returns is rejected. Refuse such a template instead of broadcasting an
    ...
    const templateHasRequiredRoots = function (rpcData: any) {
        ...
        emitErrorLog(
            'Skipping VIPSTARCOIN template at height ' +
                rpcData.height +
                ': getblocktemplate is missing hashstateroot/hashutxoroot; ' +
                'waiting for a complete template before broadcasting'
        );
        return false;
    };
